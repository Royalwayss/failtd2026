...
if(isset($_GET['mode']) && $_GET['mode'] !=''){
	...
	if($_GET['mode'] == 'update' && $_GET['id'] != '' && $title !='' && $blog_url !='' ){
		...
		$update_query  = "UPDATE  blogs SET title = '".$title."', blog_url = '".$blog_url."',short_description = '".$short_description."',blog_content = '".$blog_content."',blog_sort = '".$blog_sort."',meta_title = '".$meta_title."',meta_description = '".$meta_description."',meta_keywords = '".$meta_keywords."',published_date = '".$published_date."',read_time = '".$read_time."',status = '".$status."' WHERE id = ".$_GET['id']." "; 
	...
	if($_GET['mode'] == 'add' && $_POST['title'] != ''){
		...
		$insert_query = "INSERT INTO blogs (title,blog_url,short_description,blog_content,published_date,read_time,blog_sort,meta_title,meta_description,meta_keywords,status,created_at) VALUES ('".$title."','".$blog_url."','".$short_description."','".$blog_content."','".$published_date."','".$blog_sort."','".$read_time."','".$meta_title."','".$meta_description."','".$meta_keywords."','".$status."','".$current_time."')";
